...
const Remove = () => (
  <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M8.5 16.5C4.0816 16.5 0.5 12.9184 0.5 8.5C0.5 4.0816 4.0816 0.5 8.5 0.5C12.9184 0.5 16.5 4.0816 16.5 8.5C16.5 12.9184 12.9184 16.5 8.5 16.5ZM8.5 14.9C10.1974 14.9 11.8253 14.2257 13.0255 13.0255C14.2257 11.8253 14.9 10.1974 14.9 8.5C14.9 6.80261 14.2257 5.17475 13.0255 3.97452C11.8253 2.77428 10.1974 2.1 8.5 2.1C6.80262 2.1 5.17475 2.77428 3.97452 3.97452C2.77428 5.17475 2.1 6.80261 2.1 8.5C2.1 10.1974 2.77428 11.8253 3.97452 13.0255C5.17475 14.2257 6.80262 14.9 8.5 14.9ZM8.5 7.3688L10.7624 5.1056L11.8944 6.2376L9.6312 8.5L11.8944 10.7624L10.7624 11.8944L8.5 9.6312L6.2376 11.8944L5.1056 10.7624L7.3688 8.5L5.1056 6.2376L6.2376 5.1056L8.5 7.3688Z" fill="#E84300"/>
  </svg>
);
    ...
    <div className="w-full min-w-320px">
      ...
      >
        ...
        <div className="flex flex-col t:flex-row gap-5px items-center">
          <Input className="min-w-280px" placeholder="Search for a nonprofit or keyword" onChange={(value) => setNameFilter(value)} />
      ...
      <div className="w-full bg-gradient-to-b from-cream to-white px-20px t:px-30px d:px-120px pt-84px t:pt-88px d:pt-110px">
        <div className="min-w-280px t:max-w-1140px mx-auto flex flex-col d:flex-row gap-35px items-start">
          ...
          <div id="filters" className={`rounded-24px shadow-0-6-24 border-1px border-search-border border-opacity-20 bg-white p-15px select-none`}>
            ...
            <Input className="mt-31px d:mt-39px" placeholder="Browse by location" onChange={(value) => setLocationFilter(value)} />

            <div className="mt-23px">Causes</div>
            <div className={`grid grid-cols-1 t:grid-cols-3 d:grid-cols-1 d:w-270px gap-10px overflow-hidden ${!isFilterExpanded ? "h-327px t:h-200px d:h-467px" : ""}`}>
            ...
            {!isFilterExpanded && (
              <div className="relative t:col-span-3 d:col-span-1 h-60px -mt-55px -mx-20px -mb-20px ">
                <div className="absolute inset-0 transparent-gradient rounded-b-24px m-5px bottom-0 pt-10px">
                  ...
                  <Button className="mx-auto rounded-24pxi text-12px leading-18px flex items-center gap-5px" onClick={() => setFilterExpanded(true)}>
                    Load more
          ...
          <div id="results" className="flex-1 flex flex-col">
            ...
            <div className="flex flex-col border-b-2px border-gray-100 select-none">
              {!nameFilter && !locationFilter && interestsFilter.length === 0 ? (
                <div>
                  Top Nonprofits
                </div>
              ) : (
                ...
                <>
                  <div>Search results and top nonprofits for:</div>
                  <div className="flex flex-row flex-wrap gap-x-20px gap-y-5px font-bold text-14px leading-21px">
                    {locationFilter && (
                      <div className="flex items-center gap-2px" onClick={() => setLocationFilter(undefined)}>
                    ...
                    {interestsFilter.map((interest) => {
                      ...
                      const name = t.join(" ");
                      return (
                        <div key={name} className="flex items-center gap-2px cursor-pointer"
                             onClick={() => setInterestsFilter(prev => [...prev.filter(i => i !== interest)])}>
                          <span>{name}</span>
                          <Remove/>
                        </div>
                      );
